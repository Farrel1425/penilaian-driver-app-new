    ...
    <section class="user-list-card vehicle-list-card">
        ...
        <div class="user-table-scroll">
            <table class="user-list-table">
                <thead><tr><th>No</th><th>Foto</th><th>Nama</th><th>Email</th><th>Role</th><th class="user-status-heading">Status</th><th class="user-actions-heading">Aksi</th></tr></thead>
                <tbody>
                    @forelse ($users as $user)
                        <tr>
                            ...
                            <td class="user-status-cell"><x-admin.status-badge :tone="$user->status === \App\Models\User::STATUS_ACTIVE ? 'success' : 'danger'">{{ $user->status === \App\Models\User::STATUS_ACTIVE ? 'Aktif' : 'Nonaktif' }}</x-admin.status-badge></td>
                            <td class="user-actions-cell"><div class="table-row-actions"><a href="{{ route('admin.users.show', $user) }}" title="Lihat detail {{ $user->name }}" aria-label="Lihat detail {{ $user->name }}"><x-lucide-eye /></a><a href="{{ route('admin.users.edit', ['user' => $user, 'return_to' => 'index']) }}" title="Edit {{ $user->name }}" aria-label="Edit {{ $user->name }}"><x-lucide-pencil /></a><form method="POST" action="{{ route('admin.users.destroy', $user) }}" data-delete-confirm data-no-loading data-delete-name="Akun admin {{ $user->name }}" data-delete-description="Akun ini akan dihapus. Sistem tetap melindungi akun yang sedang digunakan dan minimal satu admin aktif.">@csrf @method('DELETE')<button type="submit" title="Hapus {{ $user->name }}" aria-label="Hapus {{ $user->name }}"><x-lucide-trash-2 /></button></form></div></td>
                        </tr>
                    @empty
