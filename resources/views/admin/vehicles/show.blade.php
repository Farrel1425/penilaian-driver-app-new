        ...
        <div class="vehicle-detail-side">
            ...
            <x-admin.panel title="Aksi Kendaraan">
                <div class="vehicle-record-actions">
                    ...
                    <form method="POST" action="{{ route('admin.vehicles.toggle-status', $vehicle) }}" @if ($vehicle->status === 'active') data-confirm data-no-loading data-confirm-title="Nonaktifkan kendaraan?" data-confirm-description="Kendaraan {{ $vehicle->police_number }} tidak dapat digunakan untuk penilaian sampai diaktifkan kembali." data-confirm-label="Nonaktifkan" data-confirm-tone="primary" data-confirm-icon="power" @endif>@csrf @method('PATCH')<button @class(['vehicle-status-action', 'is-active' => $vehicle->status === 'active']) type="submit"><x-lucide-power aria-hidden="true" /><span>{{ $vehicle->status === 'active' ? 'Nonaktifkan Kendaraan' : 'Aktifkan Kendaraan' }}</span></button></form>
